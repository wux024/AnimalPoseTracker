...
from .utils import save_one_box

from .basetrack import TrackState
from .byte_tracker import BYTETracker, STrack
from .matching import iou_distance, embedding_distance, fuse_score
from .gmc import GMC
...
class ReID:
    """encoder for re-identification."""

    def __init__(self, preprocess, inference):
    ...
    def __call__(self, img, dets):
        ...
        crops = [save_one_box(xyxy, img, square=True) for xyxy in dets]
        # Extract embeddings
        crops = [self.preprocess(crop) for crop in crops]
        crops = np.array(crops)

        feats = self.inference(crops)
